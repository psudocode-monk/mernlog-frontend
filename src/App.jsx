    ...
    <div className="relative min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center px-4">
      ...
      <div className="w-full max-w-md bg-slate-900/80 border border-emerald-500/30 rounded-2xl shadow-xl p-6 sm:p-8 backdrop-blur">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={!streakDay || !topicName || !projectUrl}
            className={`mt-2 w-full inline-flex items-center justify-center rounded-xl px-4 py-2.5 
    text-sm sm:text-base font-semibold text-slate-950 shadow-lg transition
    ${
      !streakDay || !topicName || !projectUrl
        ? "bg-emerald-500/40 cursor-not-allowed opacity-50 shadow-none"
        : "bg-emerald-500 shadow-emerald-500/30 hover:bg-emerald-400 active:bg-emerald-500 cursor-pointer"
    }
  `}
          >
